...
export default class MusicBackCommand {
	...
	async backHandler(
		...
		if (!queue.history.previousTrack) return interaction.editReply({ content: `:x: Es gibt kein vorheriges Lied <${interaction.member}...` })

		await queue.history.back()

		const backEmbed = new EmbedBuilder()
			.setAuthor({ name: await `:white_check_mark: Spiele vorheriges Lied...` })
